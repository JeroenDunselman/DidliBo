...
//  DidliBo
//
//  Created by Jeroen Dunselman on 07/04/2018.
...
//            g.addSubview(blur)
    ...
    @objc func play(_ sender:UIPanGestureRecognizer){
        ...
        if mode == "song" {
            ...
            if((velocity.x > 0 && currentVelocity.x < 0) || (velocity.x < 0 && currentVelocity.x > 0)) {
                //trigger next note
                sequenceIndex += 1
                self.playNextNote()
    ...
    func handlePan(pos: CGPoint) {
        ...
        let chordFromLocation = min(
            Int((pos.y/self.view.bounds.size.height) * CGFloat(chordZonesCount)),
            chordZonesCount - 1)
        ...
        //oct changes
        let octaveFromLocation = Int((pos.x / self.view.bounds.size.width) * CGFloat(octaveZonesCount))
